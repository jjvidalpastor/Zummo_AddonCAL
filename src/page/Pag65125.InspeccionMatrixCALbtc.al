page 65125 "Inspeccion Matrix_CAL_btc"
{
    Caption = 'Inspeccion Matrix', Comment = '"Inspeccion Matrix"';
    DeleteAllowed = false;
    InsertAllowed = false;
    LinksAllowed = false;
    PageType = List;
    ...
    var
        RecLin: Record "Lin inspe eval_CAL_btc";
    begin
        Clear(Resultado);
        Clear(RecLin);
        RecLin.SetRange("Origen inspección", "Origen inspección");
        //BEGIN FJAB 311019 Cambio campos
        //RecLin.SetRange("No. inspección", "No.");
        ...
        if RecLin.FindSet() then
            if RecLin."Tipo control" in [RecLin."Tipo control"::"Condición"] then
                Resultado := Format(RecLin."Resultado condición")
            else
                if RecLin."Tipo control" in [RecLin."Tipo control"::Valor, RecLin."Tipo control"::Puntaje] then
                    Resultado := Format(RecLin."Resultado valor")
                else
                    Resultado := Format(RecLin."Resultado texto");
        exit(Resultado);
    end;
}
