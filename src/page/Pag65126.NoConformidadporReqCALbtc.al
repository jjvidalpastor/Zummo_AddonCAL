page 65126 "No Conformidad por Req_CAL_btc"
{
    DeleteAllowed = false;
    InsertAllowed = false;
    LinksAllowed = false;
    ...
    layout
    {
        area(content)
        {
            group(Options)
            {
                ...
                field(MostrarDefDescripcion; MostrarDefDescripcion)
                {
                    Caption = 'Mostrar nombre defecto';
                    ApplicationArea = All;
                }
            }
    ...
    actions
    {
        area(processing)
        {
            action(ShowMatrix)
            {
                ...
                trigger OnAction()
                var
                    MatrixForm: Page "No Conformidad Matrix_CAL_btc";
                begin
                    Clear(MatrixForm);
                    MatrixForm.Load(MATRIX_ColumnCaptions, MATRIX_PrimaryKeyFirstColInSet, MATRIX_CurrSetLength, MostrarDefDescripcion);
                    MatrixForm.RunModal();
                end;
            }
            action("Next Set")
